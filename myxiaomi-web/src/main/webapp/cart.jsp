...
<script type="text/javascript">
	function pNum(pid,p,no){
		...
		$.ajax({
			url:"${pageContext.request.contextPath}/cartservlet?method=updateCartNum&pid="+pid+"&num=1&price="+p,
			type:"post",
			success:function(){
				location.href = "${pageContext.request.contextPath}/cartservlet?method=getCart";
			},
			error:function(){
				alert("服务器异常");
			}
	...
	function mNum(pid,p,no){
		...
		$.ajax({
			url:"${pageContext.request.contextPath}/cartservlet?method=updateCartNum&pid="+pid+"&num="+num+"&price="+p,
            type:"get",
			success:function(){
				location.href = "${pageContext.request.contextPath}/cartservlet?method=getCart";
			},
	...
	function clearCart(pid){
        if(confirm("确定要清空吗")){
            $.ajax({
                url:"${pageContext.request.contextPath}/cartservlet?method=clearCart",
                method:"get",
                success:function(){
                    location.href = "${pageContext.request.contextPath}/cartservlet?method=getCart";
                },
